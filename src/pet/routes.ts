...
export function initModule(app: express.Express) {
  ...
  app
    ...
    //mascotas perdidas del usuario
    app
    .route("/v1/lostpet")
    .get(onlyLoggedIn, findPetLostByCurrentUser)

    //update mascota perdida del mismo duenio
    app
    .route("/v1/lostpet/:petId")
    .post(onlyLoggedIn, updateLostMyPet);


    //traer todas las mascotas perdidas
    app
      .route("/v1/alllostpet")
      .get(onlyLoggedIn,findAllLostPets);
...
async function findPetLostByCurrentUser(req: ISessionRequest, res: express.Response) {
  const result = await service.findByCurrentUser(req.user.user_id);
  res.json(result.map(u => {
    return {
      id: u.id,
      ...
      birthDate: u.birthDate,
      lost: u.lost,
      idUserFind: u.userFind.toString()
    };
  }));
}

async function updateLostMyPet(req: ISessionRequest, res: express.Response) {
  const result = await service.updateLostMyPet(req.params.petId, req.user.user_id, req.body);
  res.json({
    id: result.id,
    name: result.name,
    description: result.description,
    birthDate: result.birthDate,
    lost: result.lost,
    idUserFind: result.userFind
  });
}
  async function findAllLostPets(req: ISessionRequest, res: express.Response) {
    console.log('entro a la ruta')
    const result = await service.findLostPetAllUser(req.user.user_id);
    res.json(result.map(u => {
      return {
        id: u.id,
        name: u.name,
        description: u.description,
        province: u.provinceObject,
        birthDate: u.birthDate,
        lost: u.lost,
        idUserFind: u.userFind.toString()
      };
